...
class ExplainableAIScorer:
    ...
    def calculate_data_volume_score(self, n_measurements: int) -> Tuple[float, str]:
        """
        Factor 1: Assess data volume (do we have enough history?).

        Logic:
        - Absolute minimum for trend detection: ~10 measurements
        - Good dataset for patterns: 20-30 measurements
        - Excellent: 40+ measurements

        Args:
            n_measurements (int): Number of historical measurements
    ...
    def calculate_model_agreement_score(
        ...
        """
        Factor 2: Assess model agreement (do all methods agree?).

        High agreement = robust prediction
        Low agreement = ambiguous/noisy data

        Args:
            ensemble_forecast (float): The ensemble prediction
            individual_predictions (Dict): Individual method predictions
    ...
    def calculate_stability_score(self, measurements: List[float]) -> Tuple[float, str]:
        """
        Factor 4: Assess patient stability (how chaotic is the data?).

        Stable patients = predictable future
        Chaotic patients = unpredictable
    ...
    def calculate_confidence(
        ...
        """
        Calculate overall confidence score from all 4 factors.
        ...
        Args:
            measurements (List[float]): Historical measurements
            ensemble_forecast (float): Ensemble prediction
            individual_predictions (Dict): Individual method predictions

        Returns:
            ConfidenceScore: Dataclass with all confidence components
        """
